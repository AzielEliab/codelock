...
function openapiDoc() {
  return {
    ...
    info: {
      ...
      description: MOTTO + " Source is never mutated. CodeLock mode requires the exact gate phrase. Suite mesh /v1/mesh/* PROXY to aziel-runtime (AZIEL_RUNTIME). Default OFF. QNM-BUILD-1.0 live|locked|isolated. QNS-CD-1.0 photon QNS1 packet transfer (hub cite / Worker mesh cross-map only; no public qnsd proxy). No Node Gate. No auto-heal. Not anonymity. Aziel Eliab only.",
    },
    servers: [{ url: BASE }],
    paths: {
...
function mcpDocs() {
  return {
    ...
    note: "GET docs here. Canonical agent path is the catalog MCP on aziel-runtime (FragGate slug codelock). Catalog MCP mesh_* + FragGate slug=mesh. This Worker /v1/mesh/* PROXY to aziel-runtime via AZIEL_RUNTIME. Suite mesh default OFF. QNM rollup live|locked|isolated. QNS-CD-1.0 hub cite / Worker mesh cross-map only. No public qnsd proxy. No Node Gate. No auto-heal. Not anonymity.",
    kv_increment: false,
...
export async function handleRuntime(request, url, env) {
  ...
  if (path === "/v1/health" && request.method === "GET") {
    return runtimeJson({
      ...
      source_mutated: false,
      mesh: meshPointer(),
      note: "Hosted /v1 does not increment downloads. Suite mesh /v1/mesh/* PROXY to aziel-runtime. Default OFF. QNM-BUILD-1.0. QNS-CD-1.0 hub cite / Worker mesh cross-map only. No public qnsd proxy. No Node Gate. No auto-heal.",
    });
  }
